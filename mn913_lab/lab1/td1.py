...
def manipulation_1(k: int = 10, steps: int = 1000) -> None:
...
def manipulation_2(k: int = 10, steps: int = 1000) -> None:
    ...
    for i, eps in enumerate(eps_configs):
        ...
        axes[1].bar(np.linspace(0, k - 1, k) + bar_width * (i - 1), selections,
                    width=bar_width, label=label, zorder=3)
...
def manipulation_3(k: int = 10, steps: int = 1000) -> None:
    print('----- Manipulation 3 -----')
    fig, axes = plt.subplots(1, 2, figsize=(8, 4))

    # part 1
    for beta in np.linspace(0, 1, 6):
        axes[0].plot([1 / (1 + n * beta) for n in range(200)], label=f'$\\beta={beta:.1f}$')
    axes[0].legend()
    axes[0].grid()
    axes[0].set_xlabel('$N$')
...
def manipulation_4(k: int = 10, steps: int = 1000) -> None:
...
def main(k: int = 10, steps: int = 1000):
